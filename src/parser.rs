...
impl<'a> Parser<'a> {
    ...
    fn parse_function(&mut self) -> Result<Statement, String> {
        ...
        Ok(Statement::Function(FunctionDef {
            ...
            generic_params: vec![], // Default to no generic parameters
        }))
    }
    ...
    ///
    /// # Returns
    /// A Result containing a vector of generic parameters if successful, or an error string
    fn parse_generics(&mut self) -> Result<Vec<GenericParam>, String> {
        ...
        loop {
            if let Token::Identifier(name) = self.current_token().clone() {
                // Create a GenericParam with the name and empty bounds
    ...
    fn parse_enum(&mut self) -> Result<Statement, String> {
        ...
        // Parse enum variants
        while !matches!(self.current_token(), Token::RightBrace) && !matches!(self.current_token(), Token::Eof) {
            let variant_name = if let Token::Identifier(name) = self.current_token().clone() {
            ...
            let variant_data = if matches!(self.current_token(), Token::LeftParen) {
                ...
                if !matches!(self.current_token(), Token::RightBrace) {
                    ...
                    fields.push(FieldDef {
                        ...
                        access_modifier: AccessModifier::Public, // Default to public for struct variants
                        is_mutable: false,
                        is_static: false,
                        default_value: None,
                    });
    ...
    /// Parses a type alias definition: type AliasName = SomeType
    ...
    /// Parses generic parameters: <T: Trait1 + Trait2, U: Trait3>
    fn parse_generic_params(&mut self) -> Result<Vec<GenericParam>, String> {
        ...
        let mut params = Vec::new();
        loop {
            let param_name = if let Token::Identifier(name) = self.current_token().clone() {
                self.advance(); // consume identifier
                name
            } else {
                return Err("Expected generic parameter name".to_string());
            ...
            if matches!(self.current_token(), Token::Colon) {
                ...
                // Parse first bound
                if let Token::Identifier(bound_name) = self.current_token().clone() {
                    bounds.push(bound_name);
                    self.advance(); // consume bound name
                } else {
                    return Err("Expected trait bound after ':'".to_string());
                }

                // Parse additional bounds: + Trait2 + Trait3
                while matches!(self.current_token(), Token::Plus) {
                    self.advance(); // consume '+'
                    if let Token::Identifier(bound_name) = self.current_token().clone() {
                        bounds.push(bound_name);
                        self.advance(); // consume bound name
            ...
            // Check for default type: T = SomeType
            if matches!(self.current_token(), Token::Assign) {
                self.advance(); // consume '='
