...
pub mod shell;
pub mod additional_core_features;
/// TUI module providing text-based user interface capabilities
/// with support for keyboard and mouse interactions
pub mod tui;
/// Checkbox component for TUI applications
pub mod checkbox_component;
/// Slider component for TUI applications
pub mod slider_component;
/// Zig influences module providing concepts inspired by the Zig programming language
/// with explicit control flow, manual memory management, and compile-time execution
pub mod zig_influences;
...
/// LLVM code generation module for compiling Logos to LLVM IR
pub mod llvm_code_gen;

/// Optimization passes for the Logos compiler
pub mod optimization_passes;

/// Bytecode generator for the Logos compiler
pub mod bytecode_generator;

/// Debug information generation for the Logos compiler
pub mod debug_info_gen;

/// REPL for interactive debugging and code execution
pub mod repl;
...
pub fn execute(source: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Import the Value from runtime to shadow the one from type_system
    use crate::runtime::Value;

    // Try to call the main function if it exists
    use crate::runtime::Value as RuntimeValue;

    if let Some(value) = runtime.env.get("main").cloned() {
        if let RuntimeValue::Function(_, _, body, closure_env) = value {
            // Create a new environment for the main function call
            let mut main_runtime = crate::runtime::Runtime::new();
            main_runtime.env = closure_env; // Use the closure environment

            // Evaluate the main function body
            for stmt in &body {
                main_runtime.eval_statement(stmt)?;
            }
        }
    }
