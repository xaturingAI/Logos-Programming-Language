...
// Enhanced effect handler for algebraic effects with continuation support
pub type EffectHandlerFn = dyn Fn(Vec<String>, Continuation) -> Result<String, EffectsError> + Send + Sync;
...
impl HandlerFunction {
    fn call(&self, args: Vec<String>, cont: Continuation) -> Result<String, EffectsError> {
        (self.0)(args, cont)
    }
}

// Continuation type for resuming computation after effect handling
pub type Continuation = Rc<dyn Fn() -> Result<String, EffectsError> + Send + Sync>;
...
impl EffectHandler {
    ...
    where
        F: Fn(Vec<String>, Continuation) -> Result<String, EffectsError> + Send + Sync + 'static,
    {
        self.handlers.insert(operation_name, HandlerFunction(Box::new(handler)));
    }

    pub fn handle(&self, operation_name: &str, args: Vec<String>, cont: Continuation) -> Option<Result<String, EffectsError>> {
        self.handlers.get(operation_name).map(|handler| handler.call(args, cont))
    }
}
...
impl EffectContext {
    ...
    pub fn handle_effect(&self, effect_name: &str, operation_name: &str, args: Vec<String>, cont: Continuation) -> Option<Result<String, EffectsError>> {
        for handler in &self.handlers {
            if handler.effect_name == effect_name {
                if let Some(result) = handler.handle(operation_name, args.clone(), cont.clone()) {
                    return Some(result);
                }
            }
        }
...
// Enhanced effect system with proper effect handling and resumption
pub struct EffectfulComputation<T> {
    value: Option<T>,
    effects: Vec<Effect>,
    continuation: Option<Rc<dyn Fn() -> EffectfulComputation<T>>>,
}

// Manual Debug implementation that skips the continuation field
impl<T: std::fmt::Debug> std::fmt::Debug for EffectfulComputation<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EffectfulComputation")
...
impl<T> EffectfulComputation<T> {
    ...
    pub fn with_effect(effect: Effect, continuation: Rc<dyn Fn() -> EffectfulComputation<T>>) -> Self {
        EffectfulComputation {
            value: None,
            effects: vec![effect],
    ...
    pub fn resume(self) -> Option<EffectfulComputation<T>> {
        self.continuation.map(|cont| cont())
    }
}

// Effect handler system with proper resumption
pub struct EffectSystem {
    handlers: HashMap<String, Rc<dyn Fn(&str, Vec<String>) -> Result<String, EffectsError>>>,
}
...
impl EffectSystem {
    ...
    pub fn register_handler<F>(&mut self, effect_name: &str, handler: F)
    where
        F: Fn(&str, Vec<String>) -> Result<String, EffectsError> + 'static,
    {
        self.handlers.insert(effect_name.to_string(), Rc::new(handler));
    }

    pub fn handle_effect(&self, effect_name: &str, operation: &str, args: Vec<String>) -> Result<String, EffectsError> {
        if let Some(handler) = self.handlers.get(effect_name) {
            handler(operation, args)
        } else {
            Err(EffectsError::new(&format!("No handler for effect: {}", effect_name)))
        }
    }
}
...
mod tests {
    ...
    fn test_effect_system() {
        ...
        effect_system.register_handler("Console", |op, args| {
            match op {
                "print" => Ok(format!("Printed: {}", args.join(" "))),
                _ => Err(EffectsError::new("Unknown operation")),
            }
        });

        let result = effect_system.handle_effect("Console", "print", vec!["Hello".to_string(), "World".to_string()]);
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), "Printed: Hello World");
    }
}