...
pub enum Statement {
    ...
    Enum(EnumDef),          // Enum definition
    TypeAlias(TypeAliasDef), // Type alias definition
}
...
pub enum Type {
    ...
    GenericWithBounds {                   // Generic type with trait bounds
        name: String,
        bounds: Vec<String>,              // Trait bounds for the generic type
    },
...
impl fmt::Display for Type {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Type::GenericWithBounds { name, bounds } => {
                if bounds.is_empty() {
                    write!(f, "{}", name)
                } else {
...
pub struct FunctionDef {
    ...
    pub generic_params: Vec<GenericParam>, // Generic type parameters with bounds
}

/// Represents a generic parameter with optional trait bounds
#[derive(Debug, Clone, PartialEq)]
pub struct GenericParam {
    pub name: String,                     // Name of the generic parameter
    pub bounds: Vec<String>,              // Trait bounds for the generic parameter
    pub default_type: Option<Type>,       // Default type if any
}
...
pub struct ClassDef {
    ...
    pub generics: Vec<GenericParam>,      // Generic type parameters with bounds
...
/// Represents an enum definition (algebraic data type)
#[derive(Debug, Clone, PartialEq)]
pub struct EnumDef {
    pub name: String,                     // Enum name
    pub variants: Vec<VariantDef>,        // Enum variants
    pub access_modifier: AccessModifier,  // Access level (public, private, etc.)
    pub generics: Vec<GenericParam>,      // Generic type parameters with bounds
}

/// Represents an enum variant (can have associated data)
#[derive(Debug, Clone, PartialEq)]
pub struct VariantDef {
    pub name: String,                     // Variant name
    pub data: Option<VariantData>,        // Associated data (if any)
}

/// Represents the data associated with an enum variant
#[derive(Debug, Clone, PartialEq)]
pub enum VariantData {
    Tuple(Vec<Type>),                    // Tuple variant: MyEnum::Variant(Type1, Type1, ...)
    Struct(Vec<FieldDef>),               // Struct variant: MyEnum::Variant { field1: Type1, field2: Type2 }
    Unit,                                // Unit variant: MyEnum::Variant
}
...
pub struct TypeAliasDef {
    ...
    pub aliased_type: Type,               // The type being aliased
    pub access_modifier: AccessModifier,  // Access level (public, private, etc.)
    pub generics: Vec<GenericParam>,      // Generic type parameters with bounds
}
...
pub enum Pattern {
    ...
    Enum(String, String, Option<Vec<Pattern>>), // Enum pattern: Enum::Variant(patterns...) - enum name, variant name, optional sub-patterns
    Range(i64, i64),                     // Range pattern: low..high
    Irrefutable(Box<Pattern>),           // Irrefutable pattern: always matches
    Guard(Box<Pattern>, Box<Expression>), // Pattern with guard: pattern if condition
}
