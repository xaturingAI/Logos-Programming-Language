    ...
    match cli.command {
        Commands::Run { file, debug, profile, verbose, timeout: _, memory_profile } => {
            ...
            if verbose {
                ...
                println!("Profile: {}", if profile { "enabled" } else { "disabled" });
                println!("Memory Profile: {}", if memory_profile { "enabled" } else { "disabled" });
            }

            return Ok(());
        },
        
        Commands::Build { file, output, release, opt_level, target: _, debug_symbols, verbose } => {
            if verbose {
            ...
            std::fs::write(&output, "PLACEHOLDER_EXECUTABLE_CONTENT")
